...
func makeDataSourcePipelinePlanV200(
	subtaskMetas []plugin.SubTaskMeta,
	scopeDetails []*srvhelper.ScopeDetail[models.FreshreleaseBoard, models.FreshreleaseScopeConfig],
	...
	for i, scopeDetail := range scopeDetails {
		...
		// construct task options for Freshrelease
		task, err := helper.MakePipelinePlanTask(
			"freshrelease",
			subtaskMetas,
			scopeConfig.Entities,
			FreshreleaseTaskOptions{
...
func makeScopesV200(
	...
	for _, scopeDetail := range scopeDetails {
		freshreleaseBoard, scopeConfig := scopeDetail.Scope, scopeDetail.ScopeConfig
		// add board to scopes
		if utils.StringsContains(scopeConfig.Entities, plugin.DOMAIN_TYPE_TICKET) {
			domainBoard := &ticket.Board{
				DomainEntity: domainlayer.DomainEntity{
					Id: didgen.NewDomainIdGenerator(&models.FreshreleaseBoard{}).Generate(freshreleaseBoard.ConnectionId, freshreleaseBoard.BoardId),
				},
				Name: freshreleaseBoard.Name,
				Url:  freshreleaseBoard.Self,
				Type: freshreleaseBoard.Type,
			}
