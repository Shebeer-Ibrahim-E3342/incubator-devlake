...
func (scope20240619) TableName() string {
	return "_tool_freshrelease_boards"
}

type params20240619 struct {
	ConnectionId uint64
	BoardId      uint64
}
...
func (script *addRawParamTableForScope) Up(basicRes context.BasicRes) errors.Error {
	...
	return migrationhelper.CopyTableColumns(basicRes,
		scope20240619{}.TableName(),
		scope20240619{}.TableName(),
		func(src *scope20240619) (*scope20240619, errors.Error) {
			src.RawDataTable = "_raw_freshrelease_scopes"
			src.RawDataParams = string(errors.Must1(json.Marshal(&params20240619{
			...
			where := dal.Where("id = ?", fmt.Sprintf("freshrelease:FreshreleaseBoard:%v:%v", src.ConnectionId, src.BoardId))
...
func (*addRawParamTableForScope) Version() uint64 {
	// return 20240619000002
	return 20240208000002
}

func (script *addRawParamTableForScope) Name() string {
	return "populated _raw_data columns for freshrelease boards"
}
