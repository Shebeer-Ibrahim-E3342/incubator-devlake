...
const RAW_EPIC_TABLE = "freshrelease_api_epics"
...
var CollectEpicsMeta = plugin.SubTaskMeta{
	...
	Description:      "collect Freshrelease epics from all boards, supports both timeFilter and diffSync.",
...
func CollectEpics(taskCtx plugin.SubTaskContext) errors.Error {
	...
	batchSize := 100
	if data.FreshreleaseServerInfo.DeploymentType == models.DeploymentServer && len(data.FreshreleaseServerInfo.VersionNumbers) == 3 && data.FreshreleaseServerInfo.VersionNumbers[0] <= 8 {
		batchSize = 1
	}
	...
	err = apiCollector.InitCollector(api.ApiCollectorArgs{
		...
		// Freshrelease Server returns 400 if the epic is not found
...
func GetEpicKeysIterator(db dal.Dal, data *FreshreleaseTaskData, batchSize int) (api.Iterator, errors.Error) {
	cursor, err := db.Cursor(
		dal.Select("DISTINCT epic_key"),
		dal.From("_tool_freshrelease_issues i"),
		dal.Join(`
			LEFT JOIN _tool_freshrelease_board_issues bi ON (
