...
var ConvertIssueChangelogsMeta = plugin.SubTaskMeta{
	...
	Description:      "convert Freshrelease Issue change logs",
...
func ConvertIssueChangelogs(subtaskCtx plugin.SubTaskContext) errors.Error {
	...
	var allStatus []models.FreshreleaseStatus
	err := db.All(&allStatus, dal.Where("connection_id = ?", connectionId))
	if err != nil {
		return err
	}
	statusMap := make(map[string]models.FreshreleaseStatus)
	for _, v := range allStatus {
		statusMap[v.ID] = v
	}

	issueIdGenerator := didgen.NewDomainIdGenerator(&models.FreshreleaseIssue{})
	sprintIdGenerator := didgen.NewDomainIdGenerator(&models.FreshreleaseSprint{})
	changelogIdGenerator := didgen.NewDomainIdGenerator(&models.FreshreleaseIssueChangelogItems{})
	...
	converter, err := api.NewStatefulDataConverter[IssueChangelogItemResult](&api.StatefulDataConverterArgs[IssueChangelogItemResult]{
		...
		Input: func(stateManager *api.SubtaskStateManager) (dal.Rows, errors.Error) {
			clauses := []dal.Clause{
				dal.Select("_tool_freshrelease_issue_changelog_items.*, _tool_freshrelease_issue_changelogs.issue_id, author_account_id, author_display_name, created"),
				dal.From("_tool_freshrelease_issue_changelog_items"),
				dal.Join(`left join _tool_freshrelease_issue_changelogs on (
					_tool_freshrelease_issue_changelogs.connection_id = _tool_freshrelease_issue_changelog_items.connection_id
					AND _tool_freshrelease_issue_changelogs.changelog_id = _tool_freshrelease_issue_changelog_items.changelog_id
				)`),
				dal.Join(`left join _tool_freshrelease_board_issues on (
					_tool_freshrelease_board_issues.connection_id = _tool_freshrelease_issue_changelogs.connection_id
					AND _tool_freshrelease_board_issues.issue_id = _tool_freshrelease_issue_changelogs.issue_id
				)`),
				dal.Where("_tool_freshrelease_issue_changelog_items.connection_id = ? AND _tool_freshrelease_board_issues.board_id = ?", connectionId, boardId),
			}
			if stateManager.IsIncremental() {
				since := stateManager.GetSince()
				if since != nil {
					clauses = append(clauses, dal.Where("_tool_freshrelease_issue_changelog_items.created_at >= ? ", since))
				}
			}
