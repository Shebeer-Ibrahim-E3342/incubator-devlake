...
var ExtractIssuesMeta = plugin.SubTaskMeta{
	...
	Description:      "extract Freshrelease issues",
...
func extractIssues(data *FreshreleaseTaskData, mappings *typeMappings, row *api.RawData) ([]interface{}, errors.Error) {
	...
	for _, sprintId := range sprints {
		sprintIssue := &models.FreshreleaseSprintIssue{
			ConnectionId:     data.Options.ConnectionId,
			SprintId:         sprintId,
	...
	results = append(results, &models.FreshreleaseBoardIssue{
		ConnectionId: data.Options.ConnectionId,
		BoardId:      data.Options.BoardId,
		IssueId:      issue.IssueId,
	})
	...
	for _, v := range labels {
		issueLabel := &models.FreshreleaseIssueLabel{
			IssueId:      issue.IssueId,
			LabelName:    v,
			ConnectionId: data.Options.ConnectionId,
		}
	...
	for _, v := range issuelinks {
		issueLink := &models.FreshreleaseIssueRelationship{
			ConnectionId:    data.Options.ConnectionId,
			IssueId:         issue.IssueId,
...
func getTypeMappings(data *FreshreleaseTaskData, db dal.Dal) (*typeMappings, errors.Error) {
	typeIdMapping := make(map[string]string)
	issueTypes := make([]models.FreshreleaseIssueType, 0)
	clauses := []dal.Clause{
		dal.From(&models.FreshreleaseIssueType{}),
