...
var CollectBoardFilterBeginMeta = plugin.SubTaskMeta{
	...
	Description:      "Freshrelease board filter jql checker before running",
...
func CollectBoardFilterBegin(taskCtx plugin.SubTaskContext) errors.Error {
	...
	jql := filterInfo.Jql
	var record models.FreshreleaseBoard
	err = db.First(&record, dal.Where("connection_id = ? AND board_id = ? ", data.Options.ConnectionId, data.Options.BoardId))
	if err != nil {
		return errors.Default.Wrap(err, fmt.Sprintf("error finding record in _tool_freshrelease_boards table for connection_id:%d board_id:%d", data.Options.ConnectionId, data.Options.BoardId))
	}
	...
	if syncPolicy != nil && syncPolicy.FullSync {
		if record.Jql != jql {
			...
			if err != nil {
				return errors.Default.Wrap(err, fmt.Sprintf("error updating record in _tool_freshrelease_boards table for connection_id:%d board_id:%d", data.Options.ConnectionId, data.Options.BoardId))
			}
...
func getBoardFilterId(data *FreshreleaseTaskData) (string, error) {
...
func getBoardFilterJql(data *FreshreleaseTaskData, filterId string) (*FilterInfo, error) {
