...
const RAW_USERS_TABLE = "freshrelease_api_users"

var CollectAccountsMeta = plugin.SubTaskMeta{
	Name:             "collectAccounts",
	EntryPoint:       CollectAccounts,
	EnabledByDefault: true,
	Description:      "collect Freshrelease accounts, does not support either timeFilter or diffSync.",
...
func CollectAccounts(taskCtx plugin.SubTaskContext) errors.Error {
	...
	cursor, err := db.Cursor(
		...
		dal.From("_tool_freshrelease_accounts"),
	...
	iterator, err := api.NewDalCursorIterator(db, cursor, reflect.TypeOf(models.FreshreleaseAccount{}))
	if err != nil {
		return err
	}
	queryKey := "accountId"
	...
	collector, err := api.NewApiCollector(api.ApiCollectorArgs{
		...
		Query: func(reqData *api.RequestData) (url.Values, errors.Error) {
			user := reqData.Input.(*models.FreshreleaseAccount)
			query := url.Values{}
