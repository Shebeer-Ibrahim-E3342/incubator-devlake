...
const RAW_ISSUE_TABLE = "freshrelease_api_issues"
...
var CollectIssuesMeta = plugin.SubTaskMeta{
	...
	Description:      "collect Freshrelease issues, supports both timeFilter and diffSync.",
...
func CollectIssues(taskCtx plugin.SubTaskContext) errors.Error {
	...
	apiCollector, err := api.NewStatefulApiCollector(api.RawDataSubTaskArgs{
		...
		/*
			...
			set of data to be process, for example, we process FreshreleaseIssues by Board
		*/
	...
	err = apiCollector.InitCollector(api.ApiCollectorArgs{
		...
		*/
		...
		//},
		/*
			Sometimes, we need to collect data based on previous collected data, like freshrelease changelog, it requires
...
// getTimeZone get user's timezone from freshrelease API
func getTimeZone(taskCtx plugin.SubTaskContext) (*time.Location, errors.Error) {
	data := taskCtx.GetData().(*FreshreleaseTaskData)
	connectionId := data.Options.ConnectionId
	var conn models.FreshreleaseConnection
	...
	if data.FreshreleaseServerInfo.DeploymentType == models.DeploymentServer {
		path = "api/2/user"
