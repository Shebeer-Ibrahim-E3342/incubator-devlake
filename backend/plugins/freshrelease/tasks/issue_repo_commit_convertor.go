...
var ConvertIssueRepoCommitsMeta = plugin.SubTaskMeta{
	...
	Description:      "convert Freshrelease issue repo commits",
	DomainTypes:      []string{plugin.DOMAIN_TYPE_CROSS},
}

// ConvertIssueRepoCommits is to extract issue_repo_commits from freshrelease_issue_commits, nothing difference with
...
func ConvertIssueRepoCommits(taskCtx plugin.SubTaskContext) errors.Error {
	...
	clauses := []dal.Clause{
		dal.Select("jic.*"),
		dal.From("_tool_freshrelease_issue_commits jic"),
	...
	converter, err := api.NewDataConverter(api.DataConverterArgs{
		...
		InputRowType: reflect.TypeOf(models.FreshreleaseIssueCommit{}),
		Input:        cursor,
		Convert: func(inputRow interface{}) ([]interface{}, errors.Error) {
			issueCommit := inputRow.(*models.FreshreleaseIssueCommit)
