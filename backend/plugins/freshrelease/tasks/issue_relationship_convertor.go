...
var ConvertIssueRelationshipsMeta = plugin.SubTaskMeta{
	...
	Description:      "Convert tool layer table freshrelease_issue_relationships into  domain layer table issue_relationships",
...
func ConvertIssueRelationships(taskCtx plugin.SubTaskContext) errors.Error {
	...
	cursor, err := db.Cursor(
		dal.Select("jir.*"),
		dal.From("_tool_freshrelease_issue_relationships jir"),
	...
	converter, err := helper.NewDataConverter(helper.DataConverterArgs{
		...
		InputRowType: reflect.TypeOf(models.FreshreleaseIssueRelationship{}),
		Input:        cursor,
		Convert: func(inputRow interface{}) ([]interface{}, errors.Error) {
			issueRelationship := inputRow.(*models.FreshreleaseIssueRelationship)
