...
import FreshreleaseIssueTipsImg from '@/images/freshrelease-issue-tips.png';
...
export const DevPanel = ({ connectionId, transformation, setTransformation, open, onCancel }: Props) => {
  ...
  const getRegex = async () => {
    ...
    const [success, res] = await operator(
      async () => {
        const { regex } = await API.plugin.freshrelease.generateRegex(pattern);
        const preview = await API.plugin.freshrelease.applyRegex(regex, devPanelCommits);
        return {
          regex,
          preview,
        };
  ...
  const handleSearch = async () => {
    const [success, res] = await operator(() => API.plugin.freshrelease.applicationTypes(connectionId, { key: issueKey }), {
    ...
    if (success && res) {
      ...
      message.error('Cannot find the Freshrelease issue, please input the right issue key.');
    }
  };
  ...
  const handleSubmit = async () => {
    if (step === 1 && applicationType) {
      const [success, res] = await operator(
        () => API.plugin.freshrelease.devPanelCommits(connectionId, { key: issueKey, applicationType }),
        {
          setOperating,
          hideToast: true,
        },
  ...
  return (
    <Modal
      ...
      width={820}
      centered
      title="Configure the `Application Type` and `Commit Pattern` by sending request(s) to Freshrelease."
    ...
    >
      <S.DialogBody>
        ...
        {step === 2 && (
          <>
            <Block title="Freshrelease Issue Key">{issueKey}</Block>
                  ...
                  items={[
                    {
                      ...
                      children: <img src={FreshreleaseIssueTipsImg} width="100%" alt="" />,
                    },
                  ]}
                />
              }
