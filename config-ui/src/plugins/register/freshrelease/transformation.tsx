...
export const FreshreleaseTransformation = ({ entities, connectionId, transformation, setTransformation }: Props) => {
  const [requirements, setRequirements] = useState<string[]>([]);
  const [bugs, setBugs] = useState<string[]>([]);
  const [incidents, setIncidents] = useState<string[]>([]);

  const prefix = useProxyPrefix({ plugin: 'freshrelease', connectionId });
  ...
  const { ready, data } = useRefreshData<{
    ...
    const [issueTypes, fields] = await Promise.all([API.plugin.freshrelease.issueType(prefix), API.plugin.freshrelease.field(prefix)]);
    return {
  ...
  [
    {
      ...
      children: (
        <Form labelCol={{ span: 5 }}>
          <p>
            Tell DevLake what types of Freshrelease issues you are using as features, bugs and incidents, and what field as
            `Epic Link` or `Story Points`.
          </p>
          <p>
            DevLake defines three standard types of issues: FEATURE, BUG and INCIDENT. Standardize your Freshrelease issue types
