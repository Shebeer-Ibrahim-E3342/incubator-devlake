...
export const applicationTypes = (connectionId: ID, query: { key: string }) =>
  request(`/plugins/freshrelease/connections/${connectionId}/application-types`, {
    data: query,
  });

export const devPanelCommits = (connectionId: ID, query: { key: string; applicationType: string }) =>
  request(`/plugins/freshrelease/connections/${connectionId}/dev-panel-commits`, { data: query });

export const generateRegex = (pattern: string) =>
  request('/plugins/freshrelease/generate-regex', {
    method: 'post',
    data: { pattern },
  });

export const applyRegex = (regex: string, urls: string[]) =>
  request('/plugins/freshrelease/apply-regex', {
    method: 'post',
    data: {
      regex,
      urls,
